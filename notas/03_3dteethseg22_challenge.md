...
-> DArch ya usa la curva de la arcada como contexto para el etiquetado y además lo hace de forma aprendida. Todavía no sé si DArch asume la arcada completa, si lo asume, puedo atacar por ahí. Orden de lectura acordado: TSegNet → SRF-Net → DArch → Lian.
...
El listón está en dos sitios y hay que superar los dos: IGIP (parábola + ordenación, sin entrenamiento, mejor TIR del reto) y Qiu 2022 / DArch (Bézier + GCN, aprendido).
...
- [ ] **¿DArch asume arcada completa?** Tener en cuenta esto porque en caso de que no asuma arcada completa mi propuesta se cae por completo.
